...
class EnhancedThermostatCard extends HTMLElement {
  ...
  getCardSize() {
    ...
    if (
      ...
      (this._config.door_entity ||
        ...
        this._config.dehumidifier_entity)
    ) {
      size += 4;
    }
    return size;
  ...
  _buildStaticDom() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .dial-state {
          color: var(--secondary-text-color);
          font-size: clamp(0.65rem, 8cqw, 0.95rem);
          margin-bottom: 4px;
        }
        ...
        .dial-current-suffix .unit {
          font-size: 0.36em;
        }

        .dial-current-suffix .dial-current-dec {
          font-size: 0.5em;
          margin-top: 0.1em;
        }
        ...
        <div class="dial-wrap">
          <div class="dial">
            <svg viewBox="0 0 200 200">
              <path class="dial-bg" d="M 40 160 A 80 80 0 1 1 160 160"></path>
              <path class="dial-fg" d="M 40 160 A 80 80 0 1 1 160 160"></path>
            </svg>

            <div class="dial-center">
              <div class="dial-state"></div>
              <div class="dial-current">
                <span class="dial-current-int"></span>
                <span class="dial-current-suffix">
                  <span class="unit">°C</span>
                  <span class="dial-current-dec"></span>
                </span>
              </div>
    ...
    this.shadowRoot
      .querySelector(".minus")
      .addEventListener("click", () => this._setTemp(-this._step));
  ...
  _setTemp(delta) {
    ...
    const temperature = Math.min(max, Math.max(min, rounded));
  ...
  _render() {
    ...
    targetEl.textContent =
      targetTemp !== undefined ? `${this._fmt(targetTemp)} °C` : "";
    targetEl.style.color = isOff
      ? "var(--secondary-text-color)"
      : meta.color;

    root.querySelector(".dial-controls").style.display = isOff
      ? "none"
      : "flex";

    const fg = root.querySelector(".dial-fg");
    fg.style.stroke =
      hvacMode === "off" ? "var(--disabled-text-color)" : meta.color;

    const min = stateObj.attributes.min_temp ?? 7;
    const max = stateObj.attributes.max_temp ?? 35;
    const ratio =
      targetTemp !== undefined
        ? Math.min(1, Math.max(0, (targetTemp - min) / (max - min)))
        : 0;
    ...
    const doorState = this._config.door_entity
      ? this._hass.states[this._config.door_entity]
      : null;

    const windowState = this._config.window_entity
      ? this._hass.states[this._config.window_entity]
      : null;

    const humidityState = this._config.humidity_entity
      ? this._hass.states[this._config.humidity_entity]
      : null;

    const dehumidifierState = this._config.dehumidifier_entity
      ? this._hass.states[this._config.dehumidifier_entity]
      : null;

    const doorOpen =
      doorState && ["on", "open"].includes(doorState.state);

    const windowOpen =
      windowState && ["on", "open"].includes(windowState.state);
    ...
    root.querySelector(".door-label").textContent = doorOpen
      ? "Ouverte"
      : "Fermée";

    root.querySelector(".window-label").textContent = windowOpen
      ? "Ouverte"
      : "Fermée";

    root
      .querySelector(".door-icon")
      .setAttribute(
        "icon",
        doorOpen ? "mdi:door-open" : "mdi:door-closed"
      );
    ...
    root
      ...
      .setAttribute(
        "icon",
        windowOpen ? "mdi:window-open" : "mdi:window-closed"
      );

    const humidityItem = root.querySelector(".humidity-item");
    humidityItem.style.display = humidityState ? "flex" : "none";

    root.querySelector(".humidity-label").textContent = humidityState
      ? `${humidityState.state}%`
      : "";

    const dehumidifierOn =
      dehumidifierState &&
      ["on", "true"].includes(dehumidifierState.state);
    ...
    root
      .querySelector(".dehumidifier-icon")
      .setAttribute(
        "icon",
        dehumidifierOn
          ? "mdi:air-humidifier"
    ...
    root.querySelector(".dehumidifier-label").textContent = dehumidifierOn
      ? "Marche"
      : "Arrêt";

    const modes = EnhancedThermostatCard._orderModes(
...
class EnhancedThermostatCardEditor extends HTMLElement {
  ...
  _schema() {
    return [
      ...
      {
        name: "dehumidifier_entity",
        selector: {
          entity: { domain: ["switch", "humidifier"] },
        },
      },
      {
        name: "step",
...
customElements.define(
  "enhanced-thermostat-card-editor",
  EnhancedThermostatCardEditor
);
